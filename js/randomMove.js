...
function moveSnake(){
  let moves=[]
  // check boundries
  if (snake[0].x !== ROWS - 1  && !snake.some(e => e.x === snake[0].x + 1 && e.y === snake[0].y)) {moves.push('canMoveRight')}
  if (snake[0].x !== 0 && !snake.some(e => e.x === snake[0].x - 1 && e.y === snake[0].y)) {moves.push('canMoveLeft')}
  if (snake[0].y !== COLS - 1 && !snake.some(e => e.x === snake[0].x && e.y === snake[0].y + 1)) {moves.push('canMoveDown')}
  if (
    snake[0].y !== 0 &&
